...
void deser(Node *&root, vector<int> &A, int *i, int n) {
    // if the item is NULL marker then return
    // as NULL would be already set by Node constructor
    if (A[*i] == -1) {
        (*i)++;
        return;
    }
    // if array is completely travresed
    if (*i >= n)
        return;
...
// If there are n keys, then the above solution requires n + 1 markers which may be better
// than naive solution (store inorder and preorder/postorder => storing keys twice) in situations
// where keys are big or keys have big items associated with them

// we can further optimise on space by marking leaf and internal nodes (one of them)
// so we can optimise on space used

// Company Tags: Accolite, Adobe, Amazon, Flipkart, InMobi, LinkedIn, MAQ Software, Microsoft
// Paytm, Quickr, Yahoo

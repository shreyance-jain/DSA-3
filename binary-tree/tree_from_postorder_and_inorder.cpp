...
Node *cTree(int in[], int post[], int inStrt, int inEnd, int *postIndex) {
    ...
    root->right = cTree(in, post, inIndex + 1, inEnd, postIndex);    
    root->left = cTree(in, post, inStrt, inIndex - 1, postIndex);
    
    return root;
}

/* Function to find index of value in arr[strt...end]
The function assumes value is present in in[]
complexity: linear */
int search(int in[], int strt, int end, int value) {
    int inIndex;
...
Node *buildTree(int in[], int post[], int n)
{
    int postIndex = n - 1;
    int is = 0, ie = n -1;
    Node *head = cTree(in, post, is, ie, &postIndex);
    return head;
}

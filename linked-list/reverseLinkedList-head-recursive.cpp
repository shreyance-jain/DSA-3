...
{
    // head == NULL is required when user explicitly pass NULL as head
    // head->next is our base case and it also prevent us form accessing null->next in line 121
    // if single node is remaining we return
    if (head == NULL || head->next == NULL)
        return head;
    // we reverse the linked list recursively for n-1 nodes
    // rest will be the head of the reversed n-1 nodes list (see dsa notes 23-11-19)
    Node *rest = reverseList(head->next);
    // now rest_last = head->next and rest_last->next = head
    head->next->next = head; // combining both the above code
    head->next = NULL;
